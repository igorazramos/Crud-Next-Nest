      ...
      <div className='flex gap-5 items-end'>
        ...
        <div>
          ...
          {produto.id ? (
            ...
            className="bg-blue-500 px-4 py-2 rounded-md">Alterar Produto</button>
          ) : (
            <button onClick={criarProduto} 
            className="bg-blue-500 px-4 py-2 rounded-md">Criar Produto</button>
          )}         
  ...
  function renderizarProdutos() {
    return (
      ...
      {produtos.map((produto:any) => (
        <div key={produto.id} className='flex items-center gap-2 bc-zinc-800 px-4 py-2 rouded-md'>
          <div className="flex-1">{produto.nome}</div>
          <div>{produto.preco}</div>
          <div>
            <button onClick={() => obterProdutoPorId(produto.id)} className="bg-green-500 p-2 rounded-md">Alterar</button>
          </div>
          <div>
            <button onClick={() => excluirProduto(produto.id)} className="bg-red-500 p-2 rounded-md">Excluir</button>
          </div>
        </div>
      ))}
    </div>
